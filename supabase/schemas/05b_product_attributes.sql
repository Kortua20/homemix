...
--
-- Depends on products, so it loads after 05_product_images.sql / 05a_condition_detail.sql.
--
-- Three separate lookup tables rather than one `attributes` table with a `kind`
-- discriminator. The vocabularies do not overlap, they are filtered independently, and a
-- discriminator would put a WHERE clause in front of every read for no gain. It also
-- matches condition_grades and condition_aspects, so the pattern in this schema stays one
-- pattern rather than two.
--
-- Many-to-many rather than columns on products: a sofa is "oak + linen", not one material.
-- Text arrays would work for display but not for stable ids, Georgian labels, or the
-- facet counts a filter needs.
...
create table if not exists "public"."colours" (
    ...
    "label_ka" "text" not null,
    "label_en" "text" not null,
    -- Hex for the swatch the filter UI renders. Without it every colour facet is a word,
    -- which is a poor way to choose a colour.
    "hex" "text" not null,
...
-- ---------------------------------------------------------------------------
--
-- Composite primary keys: a product cannot carry the same material twice, and re-saving is
-- an upsert rather than a duplicate row. CASCADE from products (deleting a product drops
-- its attributes), RESTRICT from the vocabularies (a term still in use cannot be deleted).
...
-- ---------------------------------------------------------------------------
--
-- Seeded here, not in seed.sql, for the same reason as condition_grades: the join tables
-- have foreign keys to these, so an empty vocabulary makes the admin unable to tag
-- anything. Append-only in practice — changing a code breaks existing product rows.

...
export async function GET(
  ...
  try {
    ...
    if (!supabaseUrl || !publishableKey || !bucket)
    ...
    const object = await createR2Client().send(
      new GetObjectCommand({ Bucket: bucket, Key: data.object_key }),
    );
    ...
    return new Response(body, {
      headers: {
        "Content-Type":
          data.content_type || object.ContentType || "application/octet-stream",
